...
const parse = <T>(value: unknown, fallback: T): T => {
  let result = value;
  for (let i = 0; i < 2 && typeof result === "string"; i += 1) {
    try {
...
async function request(query: string, variables?: Record<string, unknown>) {
  ...
  return graphqlWithDevLog(client, {
    query,
    variables,
    authMode: "userPool",
    authToken: token,
  ...
  const result = (await request(
    ...
    data?: {
      ...
      > & {
        ...
        abandonmentByMode: unknown;
        recentAbandonments: unknown;
      };
      adminEmailHistory?: unknown;
    };
    errors?: Array<{ message?: string }>;
  };
  const data = result.data?.adminDashboard;
  if (!data)
    throw new Error(
      result.errors?.[0]?.message || "Unable to load the admin dashboard.",
    );
  const activity = parse<
    | { scores?: AdminSubmission[]; notifications?: AdminNotificationHistory[] }
    | AdminSubmission[]
  >(data.recentSubmissions, []);
  return {
    ...data,
    dailyActivity: parse<DailyAdminActivity[]>(data.dailyActivity, []),
    users: parse<AdminUser[]>(data.users, []),
    abandonmentByMode: parse<AdminModeBreakdown[]>(data.abandonmentByMode, []),
    recentAbandonments: parse<AdminLifecycleEvent[]>(
      data.recentAbandonments,
      [],
    ),
    recentSubmissions: Array.isArray(activity)
      ? activity
      : (activity.scores ?? []),
    notificationHistory: Array.isArray(activity)
      ? []
      : (activity.notifications ?? []),
    emailHistory: parse<AdminEmailHistory[]>(
...
export async function sendAdminNotification(
  ...
  const result = (await request(
    "mutation SendAdminNotification($title:String!,$body:String!,$userIds:[ID!]){sendAdminNotification(title:$title,body:$body,userIds:$userIds){audienceCount sentCount failedCount}}",
    { title, body, userIds: userIds?.length ? userIds : null },
  )) as {
    data?: {
  ...
  const data = result.data?.sendAdminNotification;
  if (!data)
    throw new Error(
