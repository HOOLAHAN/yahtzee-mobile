...
export interface AdminDashboardData { totalUsers: number; completedGames: number; soloGames: number; dailyGames: number; remoteGames: number; remoteMatches: number; remoteWins: number; remoteDraws: number; gamesToday: number; gamesLast7Days: number; gamesLast30Days: number; activeUsersLast7Days: number; activeUsersLast30Days: number; averageScore: number; yahtzeesRolled: number; upperBonusesEarned: number; gameStarts: number; abandonedGames: number; resetGames: number; modeSwitchAbandons: number; remoteExits: number; staleGames: number; gameCompletionRate: number; averageAbandonRound: number; abandonmentByMode: AdminModeBreakdown[]; recentAbandonments: AdminLifecycleEvent[]; generatedAt: string; dailyActivity: DailyAdminActivity[]; users: AdminUser[]; recentSubmissions: AdminSubmission[]; notificationHistory: AdminNotificationHistory[] }
...
export async function fetchAdminDashboard(): Promise<AdminDashboardData> {
  const result = await request('query AdminDashboard { adminDashboard { totalUsers completedGames soloGames dailyGames remoteGames remoteMatches remoteWins remoteDraws gamesToday gamesLast7Days gamesLast30Days activeUsersLast7Days activeUsersLast30Days averageScore yahtzeesRolled upperBonusesEarned gameStarts abandonedGames resetGames modeSwitchAbandons remoteExits staleGames gameCompletionRate averageAbandonRound abandonmentByMode recentAbandonments generatedAt dailyActivity users recentSubmissions } }') as { data?: { adminDashboard?: Omit<AdminDashboardData, 'dailyActivity' | 'users' | 'recentSubmissions' | 'notificationHistory' | 'abandonmentByMode' | 'recentAbandonments'> & { dailyActivity: unknown; users: unknown; recentSubmissions: unknown; abandonmentByMode: unknown; recentAbandonments: unknown } }; errors?: Array<{ message?: string }> };
  ...
  return { ...data, dailyActivity: parse<DailyAdminActivity[]>(data.dailyActivity, []), users: parse<AdminUser[]>(data.users, []), abandonmentByMode: parse<AdminModeBreakdown[]>(data.abandonmentByMode, []), recentAbandonments: parse<AdminLifecycleEvent[]>(data.recentAbandonments, []), recentSubmissions: Array.isArray(activity) ? activity : activity.scores ?? [], notificationHistory: Array.isArray(activity) ? [] : activity.notifications ?? [] };
}
