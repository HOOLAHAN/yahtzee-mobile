...
export function AccountScreen({ registrationRequest = 0, onOpenAdmin, scoreSuggestionsEnabled = true, onScoreSuggestionsChange, remindersEnabled = false, reminderHour = 19, onRemindersChange, onRequestReminders, onReminderHourChange, diceAnimation = defaultDiceAnimation, onDiceAnimationChange, arcadeModeEnabled = false, onArcadeModeChange }: AccountScreenProps) {
  ...
  useEffect(() => { if (!auth.user) return; void getLifecycleEmailPreference().then((preference) => setLifecycleEmailsEnabled(preference.enabled)).catch(() => undefined); }, [auth.user?.userId]);
