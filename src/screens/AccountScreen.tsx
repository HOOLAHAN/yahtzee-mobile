...
export function AccountScreen({ registrationRequest = 0, onOpenAdmin, scoreSuggestionsEnabled = true, onScoreSuggestionsChange, remindersEnabled = false, reminderHour = 19, onRemindersChange, onRequestReminders, onReminderHourChange, diceAnimation = defaultDiceAnimation, onDiceAnimationChange, arcadeModeEnabled = false, onArcadeModeChange }: AccountScreenProps) {
  ...
  const [registrationEmailOptIn, setRegistrationEmailOptIn] = useState(false);
  ...
  useEffect(() => {
    if (!auth.user) {
      ...
      setLifecycleEmailsEnabled(false);
      emailPromptedForUser.current = null;
      return;
    }
    let active = true;
    setLifecycleEmailsHydrated(false);
    void getLifecycleEmailPreference().then((preference) => {
      ...
      setLifecycleEmailsHydrated(true);
      if (preference.consentVersion || emailPromptedForUser.current === auth.user?.userId) return;
      emailPromptedForUser.current = auth.user?.userId ?? null;
      Alert.alert(
        'Get email updates?',
        'Receive occasional game tips, reminders and Yahtzee Hub news. This is optional and you can unsubscribe at any time.',
        [
          { text: 'Not now', style: 'cancel', onPress: () => { setLifecycleEmailsBusy(true); void updateLifecycleEmailPreference(false).then((saved) => setLifecycleEmailsEnabled(saved.enabled)).catch(() => undefined).finally(() => setLifecycleEmailsBusy(false)); } },
  ...
  if (auth.user) return <ScrollView ref={scrollRef} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" automaticallyAdjustKeyboardInsets contentContainerStyle={styles.content}>
    ...
    {auth.user.role === 'ADMIN' && <Pressable accessibilityRole="button" accessibilityLabel="Open admin dashboard" onPress={onOpenAdmin} style={[styles.actionGroup, styles.adminButton]}><View style={styles.actionRow}><View style={styles.actionIcon}><Ionicons name="analytics-outline" size={21} color={colors.yellow} /></View><View style={styles.actionCopy}><Text style={styles.adminText}>Admin dashboard</Text><Text style={styles.actionDescription}>Users, games, engagement and notifications</Text></View><Ionicons name="chevron-forward" size={20} color={colors.yellow} /></View></Pressable>}
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        const step = await auth.register(email, password, username, firstName, lastName, registrationEmailOptIn);
...
const styles = StyleSheet.create({
  content: { flexGrow: 1, padding: 20, paddingBottom: 28 }, loader: { flex: 1 }, formIntro: { marginBottom: 20 }, formTitle: { color: colors.yellow, fontSize: 24, fontWeight: '900' }, formSubtitle: { color: colors.mint, fontSize: 12, lineHeight: 18, marginTop: 4 },
  ...
  actionGroup: { backgroundColor: colors.surface, borderColor: '#2d3c40', borderWidth: 1, borderRadius: 16, overflow: 'hidden' }, adminButton: { borderColor: '#666d13', backgroundColor: '#24270f', marginBottom: 12 }, adminText: { color: colors.yellow, fontSize: 16, fontWeight: '900' }, actionRow: { flexDirection: 'row', alignItems: 'center', padding: 15 }, actionIcon: { width: 38, height: 38, borderRadius: 11, backgroundColor: '#182326', alignItems: 'center', justifyContent: 'center', marginRight: 12 }, actionCopy: { flex: 1 }, actionTitle: { color: colors.white, fontSize: 16, fontWeight: '800' }, actionDescription: { color: colors.muted, fontSize: 12, marginTop: 3 }, divider: { height: 1, backgroundColor: '#263337', marginLeft: 65 },
