...
export function RealDiceScreen({ onOpenSettings }: { onOpenSettings?: () => void }) {
  ...
  if (!players.length) return <KeyboardAvoidingView style={styles.keyboardArea} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={76}><ScrollView ref={setupScrollRef} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" contentContainerStyle={styles.setup}>
    ...
    <View style={styles.setupActionRow}><Pressable onPress={startGame} style={styles.setupStartButton}><Ionicons name="play" size={18} color={colors.background} /><Text style={styles.startText}>Start Scorecard</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Choose another game" onPress={onOpenSettings} style={styles.setupSettingsButton}><Ionicons name="grid-outline" size={18} color={colors.cyan} /><Text style={styles.outlineText}>Choose Game</Text></Pressable></View>
