...
function AnimatedDie({ value, index, held, rollToken, canHold, reduceMotion, animation = defaultDiceAnimation, compact = false, accentColor, heldColor, softColor, onPress }: {
  value: DieFace; index: number; held: boolean; rollToken: number; canHold: boolean; reduceMotion: boolean; animation?: DiceAnimation; compact?: boolean; accentColor: string; heldColor: string; softColor: string; onPress: () => void;
}) {
  ...
  return <Animated.View style={[styles.dieSlot, compact && styles.previewDieSlot, { transform: [{ translateX }, { translateY: lift }, { rotate }, { scale }] }, held && styles.heldDieSlot]}>
    <Pressable
      ...
      disabled={!canHold}
      onPress={onPress}
      style={({ pressed }) => [styles.die, compact && styles.previewDie, { backgroundColor: accentColor, borderColor: accentColor, shadowColor: accentColor }, held && styles.heldDie, held && { backgroundColor: heldColor, borderColor: accentColor, shadowColor: heldColor }, pressed && styles.diePressed]}
    >
      <PipFace value={value} />
...
export function GameScreen({ chooserRequest = 0, resumeRequest = 0, dailyLaunchRequest = 0, onHeaderTitleChange, onPlayNavigationChange, scoreSuggestionsEnabled = true, diceAnimation = defaultDiceAnimation, onDiceAnimationChange = () => undefined, remindersEnabled = false, onRequestReminders, onDailyCompleted, onOpenAccount }: GameScreenProps) {
  ...
  return <View style={styles.gameContainer}>
    ...
    <View style={[styles.turnControls, dailyMode && styles.dailyTurnControls]}>
      ...
      <View style={styles.diceRow}>{dice.map((die, index) => <AnimatedDie key={index} value={die} index={index} held={held.has(index)} rollToken={rollToken} canHold={hasRolled && !complete && !isComputerTurn} reduceMotion={reduceMotion} animation={diceAnimation} accentColor={currentProfile.accent} heldColor={dailyMode ? colors.pink : colors.yellow} softColor={colors.background} onPress={() => toggleHeld(index)} />)}</View>
      ...
      <View style={styles.compactSummary}>
        ...
        {twoPlayer && <Text style={styles.compactLabel}>{computerOpponent ? 'You' : `P${currentPlayer === 1 ? 2 : 1}`} <Text style={[styles.compactValue, { color: currentPlayer === 1 ? secondPlayerProfile.score : playerProfiles[0].score }]}>{totals[currentPlayer === 1 ? 2 : 1]}</Text></Text>}
...
const styles = StyleSheet.create({
  ...
  animationIntro: { color: colors.muted, fontSize: 12, lineHeight: 17, marginTop: -3, marginBottom: 9 }, animationGrid: { gap: 8, paddingBottom: 10 }, animationChoice: { minHeight: 76, flexDirection: 'row', alignItems: 'center', gap: 11, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 14, borderWidth: 1, borderColor: '#2d3c40', backgroundColor: colors.surface }, animationChoiceSelected: { borderColor: colors.cyan, backgroundColor: '#172528' }, animationPreview: { width: 62, height: 58, alignItems: 'center', justifyContent: 'center', overflow: 'visible' }, animationCopy: { flex: 1 }, animationTitle: { color: colors.white, fontSize: 14, fontWeight: '900' }, animationTitleSelected: { color: colors.cyan }, animationDescription: { color: colors.muted, fontSize: 11, marginTop: 3 }, previewDieSlot: { width: 50, height: 50 }, previewDie: { borderRadius: 9, borderWidth: 2 },
