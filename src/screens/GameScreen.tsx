...
export function GameScreen() {
  ...
  const sendScore = async () => {
    ...
    setSubmitting(true);
    try { await submitScore(totals[1]); setSubmitted(true); showToast(`${totals[1]} points submitted to the leaderboard`); }
  ...
  return <View style={styles.gameContainer}>
    ...
    <ScrollView contentContainerStyle={[styles.content, selectedCategory && styles.contentWithLock]}>
      {complete ? <View style={styles.completeCard}>
        ...
        {hasRolled && <View style={styles.panel}><Pressable accessibilityRole="button" accessibilityState={{ expanded: showBreakdown }} onPress={() => setShowBreakdown((value) => !value)} style={styles.panelHeader}><Text style={styles.panelTitle}>All score previews</Text><Ionicons name={showBreakdown ? 'chevron-up' : 'chevron-down'} size={20} color={colors.cyan} /></Pressable>{showBreakdown && <View style={styles.breakdown}>{categories.filter((category) => !used.has(category)).map((category) => <View key={category} style={styles.breakdownRow}><Text style={styles.breakdownLabel}>{category}</Text><Text style={[styles.breakdownScore, scoreCategory(category, dice) === 0 && styles.zeroText]}>{scoreCategory(category, dice)}</Text></View>)}</View>}</View>}
...
const styles = StyleSheet.create({
  ...
  panel: { borderColor: '#2d3c40', borderWidth: 1, borderRadius: 13, backgroundColor: colors.surface, padding: 14, marginTop: 14 }, panelHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, panelTitle: { color: colors.cyan, fontSize: 16, fontWeight: '900' }, breakdown: { marginTop: 10 }, breakdownRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 }, breakdownLabel: { color: colors.mint }, breakdownScore: { color: colors.yellow, fontWeight: '800' }, zeroText: { color: colors.pink },
