...
interface GameScreenProps { chooserRequest?: number; dailyLaunchRequest?: number; onHeaderTitleChange?: (title: string) => void; scoreSuggestionsEnabled?: boolean; remindersEnabled?: boolean; onRequestReminders?: () => void; onDailyCompleted?: () => void; onOpenAccount?: () => void }

export function GameScreen({ chooserRequest = 0, dailyLaunchRequest = 0, onHeaderTitleChange, scoreSuggestionsEnabled = true, remindersEnabled = false, onRequestReminders, onDailyCompleted, onOpenAccount }: GameScreenProps) {
  const { user } = useAuth();
  ...
  return <View style={styles.gameContainer}>
    ...
    <View style={[styles.turnControls, dailyMode && styles.dailyTurnControls]}>
      ...
      <View style={styles.diceRow}>{dice.map((die, index) => <AnimatedDie key={index} value={die} index={index} held={held.has(index)} rollToken={rollToken} canHold={hasRolled && !complete && !isComputerTurn} reduceMotion={reduceMotion} accentColor={currentProfile.accent} heldColor={dailyMode ? colors.pink : colors.yellow} softColor={colors.background} onPress={() => toggleHeld(index)} />)}</View>
      ...
      {dailyMode && scores.length > 0 && !complete && (user ? <View accessibilityLiveRegion="polite" style={styles.dailyPaceCard}><View style={styles.dailyPaceIcon}><Ionicons name="stats-chart" size={17} color={colors.pink} /></View><View style={styles.dailyPaceCopy}>{dailyStandingLoading && !dailyRoundStanding ? <Text style={styles.dailyPaceTitle}>Updating today’s standing…</Text> : dailyRoundStanding ? <><Text style={styles.dailyPaceTitle}>{dailyRoundStanding.playerCount >= 5 ? `Top ${dailyRoundStanding.percentile}% after Round ${dailyRoundStanding.round}` : `#${dailyRoundStanding.rank} after Round ${dailyRoundStanding.round}`}</Text><Text style={styles.dailyPaceDetail}>{dailyRoundStanding.playerCount >= 5 ? `#${dailyRoundStanding.rank} of ${dailyRoundStanding.playerCount} players at the same stage` : `${dailyRoundStanding.playerCount} ${dailyRoundStanding.playerCount === 1 ? 'player has' : 'players have'} reached this round so far`}</Text></> : <Text style={styles.dailyPaceTitle}>Standing unavailable right now</Text>}</View>{dailyStandingLoading && dailyRoundStanding && <Ionicons name="sync" size={14} color={colors.muted} />}</View> : <Pressable onPress={onOpenAccount} style={styles.dailyPaceCard}><View style={styles.dailyPaceIcon}><Ionicons name="person-outline" size={17} color={colors.pink} /></View><View style={styles.dailyPaceCopy}><Text style={styles.dailyPaceTitle}>See how your round ranks</Text><Text style={styles.dailyPaceDetail}>Sign in to compare your pace with today’s players.</Text></View><Ionicons name="arrow-forward" size={16} color={colors.yellow} /></Pressable>)}
    </View>
...
const styles = StyleSheet.create({
  ...
  dailyPaceCard: { minHeight: 50, flexDirection: 'row', alignItems: 'center', gap: 9, marginTop: 8, paddingHorizontal: 11, paddingVertical: 8, borderRadius: 12, borderColor: '#656a13', borderWidth: 1, backgroundColor: '#25270d' }, dailyPaceIcon: { width: 32, height: 32, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: '#3a1d34' }, dailyPaceCopy: { flex: 1 }, dailyPaceTitle: { color: colors.yellow, fontSize: 11, fontWeight: '900' }, dailyPaceDetail: { color: colors.mint, fontSize: 9, lineHeight: 13, marginTop: 2 },
