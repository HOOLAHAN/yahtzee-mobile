...
import { AccessibilityInfo, Alert, Animated, AppState, Easing, Modal, PanResponder, Pressable, ScrollView, Share, StyleSheet, useWindowDimensions, View } from 'react-native';
...
export function GameScreen({ chooserRequest = 0, resumeRequest = 0, dailyLaunchRequest = 0, liveGameRequest, onHeaderTitleChange, onPlayNavigationChange, scoreSuggestionsEnabled = true, diceAnimation = defaultDiceAnimation, remindersEnabled = false, onRequestReminders, onDailyCompleted, onOpenDailyLeaderboard, onOpenAccount }: GameScreenProps) {
  ...
  const [toastKind, setToastKind] = useState<'standard' | 'lock' | 'achievement'>('standard');
  ...
  const showToast = (message: string, kind: 'standard' | 'lock' | 'achievement' = 'standard', title = 'TOP 3!') => {
    ...
    if (reduceMotion) {
      ...
      toastAnimation.current = Animated.sequence([Animated.delay(kind === 'achievement' ? 3000 : kind === 'lock' ? 700 : 1300), Animated.timing(toastOpacity, { toValue: 0, duration: 1, useNativeDriver: true })]);
    } else {
      toastAnimation.current = Animated.sequence([
        ...
        Animated.delay(kind === 'achievement' ? 3000 : kind === 'lock' ? 700 : 1450),
  ...
  const lockScore = () => {
    ...
    showToast(entry.yahtzeeBonus ? `Bonus Yahtzee! ${selectedCategory}: ${entry.score} category + ${entry.yahtzeeBonus} bonus = ${entry.score + entry.yahtzeeBonus} total` : `${selectedCategory} locked in for ${entry.score} ${entry.score === 1 ? 'point' : 'points'}`, 'lock');
  ...
  const shareScorecard = async () => {
    ...
    const resultFor = (player: Player) => {
      ...
      const lineFor = (category: Category) => {
        const entry = histories[player].find((item) => item.category === category);
        return `${categoryLabels[category].padEnd(13)} ${scoreFor(category)}${entry?.yahtzeeBonus ? ` (+${entry.yahtzeeBonus} Yahtzee bonus)` : ''}`;
      };
      const upper = upperCategories.map(lineFor).join('\n');
      const lower = categories.slice(6).map(lineFor).join('\n');
      return [
  ...
  const toastView = <Animated.View accessibilityLiveRegion="polite" pointerEvents="none" style={[styles.toast, toastKind === 'lock' && styles.lockToast, toastKind === 'achievement' && styles.achievementToast, { opacity: toastOpacity, transform: [{ translateY: toastY }, { scale: toastScale }] }]}><View style={toastKind === 'achievement' ? styles.achievementToastIcon : styles.standardToastIcon}><Ionicons name={toastKind === 'achievement' ? 'trophy' : 'checkmark'} size={toastKind === 'achievement' ? 28 : 17} color={toastKind === 'achievement' ? colors.yellow : colors.cyan} /></View><View style={styles.toastCopy}>{toastKind === 'achievement' && <Text style={styles.achievementToastTitle}>{toastTitle}</Text>}<Text style={[styles.toastText, toastKind === 'achievement' && styles.achievementToastText]}>{toastMessage}</Text></View>{toastKind === 'achievement' && <Ionicons name="sparkles" size={22} color={colors.pink} />}</Animated.View>;

  const categoryChoice = (category: Category) => {
    const entry = scores.find((item) => item.category === category);
    const eligible = hasRolled && isCategoryEligibleForRoll(category, dice, scores);
    const preview = eligible ? scoreCategoryForTurn(category, dice, scores) : 0;
    const selected = selectedCategory === category;
    const recommended = recommendedCategory === category && !entry;
    const turnBonus = eligible ? joker.bonus : 0;
    return <Pressable accessibilityRole="button" accessibilityLabel={`${category}, ${entry ? `${entry.score} points${entry.yahtzeeBonus ? ` plus ${entry.yahtzeeBonus} Yahtzee bonus` : ''}, used` : eligible ? `${preview} category points${turnBonus ? ` plus ${turnBonus} Yahtzee bonus, ${preview + turnBonus} total` : ''}` : 'unavailable for this Joker roll'}${recommended ? ', best available score' : ''}`} accessibilityState={{ disabled: !eligible || Boolean(entry) || isComputerTurn, selected }} key={category} disabled={!eligible || Boolean(entry) || isComputerTurn} onPress={() => setSelectedCategory(category)} style={[styles.category, compactGameLayout && styles.compactCategory, arcadeMode && styles.arcadeCategory, category === 'Chance' && styles.chanceCategory, (entry || (hasRolled && !eligible)) && styles.usedCategory, recommended && !isComputerTurn && styles.recommendedCategory, recommended && dailyMode && { borderColor: colors.pink, shadowColor: colors.pink }, selected && styles.selectedCategory, selected && { borderColor: currentProfile.accent, backgroundColor: currentProfile.soft, shadowColor: currentProfile.accent }]}>
      <Text numberOfLines={2} style={[styles.categoryName, entry && styles.usedText]}>{categoryLabels[category]}{entry?.yahtzeeBonus ? ' ✦' : ''}</Text><View style={[styles.scoreBadge, (turnBonus > 0 || Boolean(entry?.yahtzeeBonus)) && styles.bonusScoreBadge, { backgroundColor: currentProfile.soft }, entry && styles.usedBadge, preview === 0 && !entry && styles.zeroBadge]}><Text style={[styles.scoreBadgeText, (turnBonus > 0 || Boolean(entry?.yahtzeeBonus)) && styles.bonusScoreBadgeText, { color: currentProfile.accent }, entry && styles.usedText]}>{entry ? entry.yahtzeeBonus ? `${entry.score}+${entry.yahtzeeBonus}` : entry.score : turnBonus ? `${preview}+${turnBonus}` : preview}</Text></View>{recommended && <Ionicons name="sparkles" size={12} color={dailyMode ? colors.pink : colors.yellow} style={styles.recommendedIcon} />}
    </Pressable>;
  };
  ...
  const scorecardContent = (player: Player) => {
    ...
    <View style={styles.upperProgress} accessibilityRole="progressbar" accessibilityLabel="Upper bonus progress" accessibilityValue={{ min: 0, max: upperBonusThreshold, now: Math.min(upperSubtotals[player], upperBonusThreshold), text: bonuses[player] ? 'Upper bonus achieved' : `${upperSubtotals[player]} of ${upperBonusThreshold} points` }}><View style={styles.upperProgressTrack}><View style={[styles.upperProgressFill, bonuses[player] > 0 && styles.upperProgressComplete, { width: `${Math.min(100, (upperSubtotals[player] / upperBonusThreshold) * 100)}%` }]} /></View><View style={styles.upperProgressLabels}><Text style={styles.upperProgressText}>{Math.min(upperSubtotals[player], upperBonusThreshold)} / {upperBonusThreshold}</Text><Text style={[styles.upperProgressText, bonuses[player] > 0 && styles.bonusEarned]}>{bonuses[player] ? 'Bonus achieved!' : `${upperBonusThreshold - upperSubtotals[player]} points to go`}</Text></View></View>
    {histories[player].some((entry) => entry.yahtzeeBonus) && <View style={[styles.bonusRow, styles.bonusRowEarned]}><View style={styles.bonusCopy}><Ionicons name="sparkles" size={17} color={colors.yellow} /><Text style={styles.bonusLabel}>{histories[player].filter((entry) => entry.yahtzeeBonus).length} extra Yahtzee{histories[player].filter((entry) => entry.yahtzeeBonus).length === 1 ? '' : 's'}</Text></View><Text style={[styles.bonusValue, styles.bonusEarned]}>+{histories[player].reduce((sum, entry) => sum + (entry.yahtzeeBonus ?? 0), 0)}</Text></View>}
    <View style={styles.sheetScoreColumns}><View style={[styles.sheetScoreColumn, styles.upperScoreColumn]}><View style={styles.scoreGroupHeader}><Ionicons name="trending-up-outline" size={14} color={colors.cyan} /><Text style={[styles.scoreGroupTitle, styles.upperGroupTitle]}>Upper section</Text></View>
    {upperCategories.map((category) => { const entry = histories[player].find((item) => item.category === category); return <View key={category} style={styles.sheetScoreRow}><Text numberOfLines={1} style={styles.sheetCategory}>{category}{entry?.yahtzeeBonus ? ' ✦' : ''}</Text><Text style={[styles.sheetScore, { color: profile.score }]}>{entry ? `${entry.score}${entry.yahtzeeBonus ? ` +${entry.yahtzeeBonus}` : ''}` : '—'}</Text></View>; })}</View>
    <View style={[styles.sheetScoreColumn, styles.lowerScoreColumn]}><View style={styles.scoreGroupHeader}><Ionicons name="flash-outline" size={14} color={colors.pink} /><Text style={styles.scoreGroupTitle}>Lower section</Text></View>
    {categories.slice(6).map((category) => { const entry = histories[player].find((item) => item.category === category); return <View key={category} style={styles.sheetScoreRow}><Text numberOfLines={1} style={styles.sheetCategory}>{categoryLabels[category]}{entry?.yahtzeeBonus ? ' ✦' : ''}</Text><Text style={[styles.sheetScore, { color: profile.score }]}>{entry ? `${entry.score}${entry.yahtzeeBonus ? ` +${entry.yahtzeeBonus}` : ''}` : '—'}</Text></View>; })}</View></View>
  ...
  return <View style={styles.gameContainer}>
    ...
    <ScrollView contentContainerStyle={[styles.content, selectedCategory && styles.contentWithLock]}>
      {complete ? <View style={styles.completeCard}>
        ...
        {hasRolled && joker.active && !isComputerTurn && <View style={styles.jokerNotice}><Ionicons name="sparkles" size={16} color={colors.yellow} /><View style={styles.jokerNoticeCopy}><Text style={styles.jokerNoticeTitle}>{joker.bonus ? `BONUS YAHTZEE · +${joker.bonus}` : 'JOKER RULE · NO +100 BONUS'}</Text><Text style={styles.jokerNoticeText}>{joker.phase === 'matching-upper' ? `${joker.matchingUpper} is required and scores ${scoreCategoryForTurn(joker.matchingUpper!, dice, scores)}.` : joker.phase === 'open-lower' ? 'Choose any open Lower category. Fixed combinations receive their full score.' : 'All Lower categories are filled. Choose an open Upper category; it scores 0.'}</Text></View></View>}
        <View style={[styles.categorySection, compactGameLayout && styles.compactCategorySection, styles.upperCategorySection]}>
          <View style={styles.categorySectionHeader}><View style={styles.categorySectionTitle}><Ionicons name="trending-up-outline" size={15} color={colors.cyan} /><Text style={styles.upperCategoryTitle}>Upper section</Text></View><Text style={styles.categorySectionStatus}>{bonuses[currentPlayer] ? '+35 earned' : `${Math.max(0, upperBonusThreshold - upperSubtotals[currentPlayer])} needed`}</Text></View>
          <View style={styles.chooserBonusTrack}><View style={[styles.chooserBonusFill, bonuses[currentPlayer] > 0 && styles.chooserBonusComplete, { width: `${Math.min(100, (upperSubtotals[currentPlayer] / upperBonusThreshold) * 100)}%` }]} /></View>
          <View style={styles.chooserBonusLabels}><Text style={styles.chooserBonusText}>{upperSubtotals[currentPlayer]} / {upperBonusThreshold}</Text><Text style={styles.chooserBonusText}>Earn +{upperBonusPoints}</Text></View>
          <View style={styles.categoryGrid}>{upperCategories.map(categoryChoice)}</View>
        </View>
        <View style={[styles.categorySection, compactGameLayout && styles.compactCategorySection, styles.lowerCategorySection]}>
          <View style={styles.categorySectionHeader}><View style={styles.categorySectionTitle}><Ionicons name="flash-outline" size={15} color={colors.pink} /><Text style={styles.lowerCategoryTitle}>Lower section</Text></View><Text style={styles.categorySectionStatus}>Combinations</Text></View>
          <View style={styles.categoryGrid}>{categories.slice(6).map(categoryChoice)}</View>
        </View>
      </>}
    </ScrollView>

    {!complete && !selectedCategory && <View style={styles.actions}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Choose another game" onPress={() => setShowModeChooser(true)} style={[styles.gameActionButton, arcadeMode && styles.arcadeActionButton]}><Ionicons name="grid-outline" size={18} color={colors.mint} /><Text style={styles.gameActionText}>Games</Text></Pressable>
    ...
    {selectedCategory && !complete && !isComputerTurn && <View style={[styles.lockBar, arcadeMode && styles.arcadeLockBar, { borderColor: currentProfile.accent, shadowColor: currentProfile.accent }]}><View style={styles.lockCopy}><Text style={styles.lockLabel}>{joker.active ? `Joker → ${selectedCategory}` : selectedCategory}</Text><Text numberOfLines={2} style={[styles.lockScore, { color: currentProfile.score }]}>{joker.bonus ? `${scoreCategoryForTurn(selectedCategory, dice, scores)} category + ${joker.bonus} bonus = ${scoreCategoryForTurn(selectedCategory, dice, scores) + joker.bonus} total` : `${scoreCategoryForTurn(selectedCategory, dice, scores)} points${joker.active ? ' · no Yahtzee bonus' : ''}`}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`Lock in ${selectedCategory} for ${scoreCategoryForTurn(selectedCategory, dice, scores)} category points${joker.bonus ? ` plus ${joker.bonus} Yahtzee bonus, ${scoreCategoryForTurn(selectedCategory, dice, scores) + joker.bonus} total points` : ''}`} onPress={lockScore} style={[styles.lockButton, arcadeMode && styles.arcadeLockButton, { backgroundColor: currentProfile.accent, borderColor: currentProfile.score }]}><Ionicons name="lock-closed" size={18} color={colors.background} /><Text style={styles.lockButtonText}>{arcadeMode ? '> LOCK IN_' : 'Lock In'}</Text></Pressable></View>}
...
const styles = StyleSheet.create({
  gameContainer: { flex: 1 }, content: { flexGrow: 1, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 16 }, contentWithLock: { paddingBottom: 105 },
  ...
  primaryButton: { backgroundColor: colors.cyan, padding: 11, borderRadius: 12, alignItems: 'center', marginTop: 7 }, buttonContent: { flexDirection: 'row', alignItems: 'center', gap: 7 }, primaryText: { color: colors.background, fontWeight: '900', fontSize: 16 }, pressed: { opacity: 0.75 }, disabled: { opacity: 0.45 }, compactSummary: { minHeight: 22, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', marginTop: 5 }, compactLabel: { color: colors.muted, fontSize: 11, fontWeight: '700' }, compactValue: { color: colors.yellow, fontSize: 14, fontWeight: '900' }, compactPosition: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 5, paddingVertical: 2 }, compactPositionValue: { color: colors.pink, fontSize: 13, fontWeight: '900' },
  ...
  sectionHeadingRow: { flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 8 }, sectionTitle: { color: colors.yellow, fontSize: 19, fontWeight: '900' }, sectionSubtitle: { color: colors.muted, fontSize: 11, marginTop: 2 }, recommendedLegend: { flexDirection: 'row', alignItems: 'center', gap: 3 }, recommendedLegendText: { color: colors.yellow, fontSize: 10, fontWeight: '800' }, jokerNotice: { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 10, paddingVertical: 7, marginBottom: 7, borderRadius: 9, borderWidth: 1, borderColor: '#6d7118', backgroundColor: '#27290f' }, jokerNoticeCopy: { flex: 1 }, jokerNoticeTitle: { color: colors.yellow, fontSize: 9.5, fontWeight: '900', letterSpacing: .5 }, jokerNoticeText: { color: colors.mint, fontSize: 9, lineHeight: 12, marginTop: 1 },
  categorySection: { borderWidth: 1, borderRadius: 12, padding: 9, marginBottom: 8 }, compactCategorySection: { padding: 6, marginBottom: 5 }, upperCategorySection: { borderColor: '#315a5e', backgroundColor: '#112326' }, lowerCategorySection: { borderColor: '#56304d', backgroundColor: '#21151e' }, categorySectionHeader: { minHeight: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }, categorySectionTitle: { flexDirection: 'row', alignItems: 'center', gap: 5 }, upperCategoryTitle: { color: colors.cyan, fontSize: 12, fontWeight: '900', textTransform: 'uppercase' }, lowerCategoryTitle: { color: colors.pink, fontSize: 12, fontWeight: '900', textTransform: 'uppercase' }, categorySectionStatus: { color: colors.muted, fontSize: 9, fontWeight: '800' }, chooserBonusTrack: { height: 6, overflow: 'hidden', borderRadius: 3, backgroundColor: '#263337' }, chooserBonusFill: { height: '100%', borderRadius: 3, backgroundColor: colors.yellow }, chooserBonusComplete: { backgroundColor: colors.cyan }, chooserBonusLabels: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 3, marginBottom: 7 }, chooserBonusText: { color: colors.muted, fontSize: 8.5, fontWeight: '800' },
  categoryGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', rowGap: 6 }, category: { width: '32%', minHeight: 46, flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surface, borderColor: '#2d3c40', borderWidth: 1, paddingHorizontal: 7, paddingVertical: 7, borderRadius: 9 }, compactCategory: { minHeight: 38, paddingVertical: 4, paddingHorizontal: 6 }, chanceCategory: { marginLeft: '34%' }, recommendedCategory: { borderColor: colors.yellow, shadowColor: colors.yellow, shadowOpacity: 0.35, shadowRadius: 5 }, selectedCategory: { borderColor: colors.pink, borderWidth: 2, backgroundColor: '#34202f', shadowColor: colors.pink, shadowOpacity: 0.4, shadowRadius: 6 }, usedCategory: { opacity: 0.52, backgroundColor: '#151c1e' }, categoryName: { color: colors.mint, fontWeight: '800', fontSize: 10.5, lineHeight: 13, flex: 1, paddingRight: 3 }, usedText: { color: colors.muted }, scoreBadge: { minWidth: 23, height: 23, borderRadius: 12, backgroundColor: '#20383b', alignItems: 'center', justifyContent: 'center' }, bonusScoreBadge: { minWidth: 43, paddingHorizontal: 4 }, scoreBadgeText: { color: colors.cyan, fontWeight: '900', fontSize: 11 }, bonusScoreBadgeText: { fontSize: 9 }, zeroBadge: { backgroundColor: '#34202f' }, usedBadge: { backgroundColor: '#273034' }, recommendedIcon: { position: 'absolute', top: 2, right: 2 },
  lockBar: { position: 'absolute', zIndex: 15, left: 14, right: 14, bottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#162326', borderColor: colors.cyan, borderWidth: 1, borderRadius: 14, padding: 13, shadowColor: colors.cyan, shadowOpacity: 0.28, shadowRadius: 10, shadowOffset: { width: 0, height: 3 }, elevation: 10 }, lockCopy: { flex: 1, paddingRight: 8 }, lockLabel: { color: colors.white, fontWeight: '900' }, lockScore: { color: colors.yellow, fontWeight: '800', fontSize: 11, lineHeight: 14, marginTop: 2 }, lockButton: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.cyan, borderRadius: 10, paddingHorizontal: 15, paddingVertical: 11 }, lockButtonText: { color: colors.background, fontWeight: '900' },
  ...
  actions: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingTop: 7, paddingBottom: 3, backgroundColor: colors.background }, gameActionButton: { flex: 1, minHeight: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, borderColor: '#315a5e', borderWidth: 1, borderRadius: 11, backgroundColor: colors.surface }, gameActionText: { color: colors.mint, fontSize: 11, fontWeight: '800' }, resetText: { color: colors.muted }, protectedAction: { borderColor: '#656a13', backgroundColor: '#25270d' }, protectedActionText: { color: colors.yellow },
