...
interface StreakDay { key: string; day: string; date: string; result?: GameResult; rank?: number; playerCount?: number; today: boolean }
const recentStreakDays = (dailyResults: GameResult[], allDailyResults: GameResult[], todayKey: string, count = 14): StreakDay[] => {
  const resultsByDate = new Map(dailyResults.filter((result) => result.challengeDate).map((result) => [result.challengeDate!, result]));
  const boards = new Map<string, GameResult[]>();
  allDailyResults.forEach((result) => { if (!result.challengeDate) return; const board = boards.get(result.challengeDate) ?? []; board.push(result); boards.set(result.challengeDate, board); });
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const board = [...(boards.get(key) ?? [])].sort((a, b) => b.score - a.score || a.completedAt.localeCompare(b.completedAt));
    const rank = result ? board.findIndex((entry) => entry.id === result.id || entry.userId === result.userId) + 1 : 0;
    return { key, day: value.toLocaleDateString(undefined, { weekday: 'short' }), date: value.toLocaleDateString(undefined, { day: 'numeric', month: 'short' }), result, rank: rank || undefined, playerCount: board.length || undefined, today: key === todayKey };
  });
};
...
export function ProgressScreen({ onCreateAccount }: { onCreateAccount?: () => void }) {
  ...
  const [allDailyResults, setAllDailyResults] = useState<GameResult[]>([]);
  const load = useCallback(async () => {
    if (!user) return;
    setLoading(true); setError('');
    try { const [mine, dailyResults] = await Promise.all([fetchMyGameResults(user.userId), fetchAllDailyResults(1000)]); setResults(mine); setAllDailyResults(dailyResults); }
  ...
  const streakDays = useMemo(() => recentStreakDays(results.filter((result) => result.mode === 'DAILY'), allDailyResults, localDateKey()), [allDailyResults, results]);
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={() => void load()} tintColor={colors.cyan} />}>
    <View style={styles.dailyActivity}><View style={styles.dailyActivityHeader}><View style={styles.streakSummary}><View style={styles.flame}><Ionicons name="flame" size={20} color={colors.yellow} /></View><View><Text style={styles.eyebrow}>Daily streak</Text><Text style={styles.streak}>{streak} <Text style={styles.streakUnit}>{streak === 1 ? 'day' : 'days'}</Text></Text></View></View><View><Text style={styles.streakHistoryTitle}>Recent activity</Text><Text style={styles.streakHistoryCopy}>Score · final position</Text></View></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.timeline}>{streakDays.map((item) => <View key={item.key} style={[styles.timelineDay, item.result ? styles.timelineComplete : styles.timelineMissed, item.today && styles.timelineToday]}><View style={styles.timelineTop}><Text style={[styles.timelineWeekday, item.today && styles.timelineTodayText]}>{item.today ? 'TODAY' : item.day.toUpperCase()}</Text>{item.result ? <Ionicons name="checkmark" size={11} color={colors.cyan} /> : <View style={styles.missedMark} />}</View><Text style={styles.timelineDate}>{item.date}</Text>{item.result ? <><Text style={styles.timelineScore}>{item.result.score}</Text><Text style={styles.timelineRank}>{item.rank ? `#${item.rank} / ${item.playerCount}` : 'Recorded'}</Text></> : <Text style={styles.timelineMissedLabel}>{item.today ? 'Not played' : 'Missed'}</Text>}</View>)}</ScrollView><Text style={styles.rankNote}>Today’s position may change as more players finish.</Text></View>
...
const styles = StyleSheet.create({
  ...
  dailyActivity: { backgroundColor: colors.surface, borderColor: '#315057', borderWidth: 1, borderRadius: 16, overflow: 'hidden', paddingVertical: 11 }, dailyActivityHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 12, paddingBottom: 10 }, streakSummary: { flexDirection: 'row', alignItems: 'center', gap: 8 }, eyebrow: { color: colors.muted, fontSize: 8, fontWeight: '900', textTransform: 'uppercase' }, streak: { color: colors.yellow, fontSize: 23, lineHeight: 26, fontWeight: '900' }, streakUnit: { fontSize: 11 }, flame: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: '#303114' },
  streakHistoryTitle: { color: colors.white, fontSize: 11, fontWeight: '900', textAlign: 'right' }, streakHistoryCopy: { color: colors.muted, fontSize: 7.5, marginTop: 1, textAlign: 'right' }, timeline: { gap: 6, paddingHorizontal: 11 }, timelineDay: { width: 66, minHeight: 72, borderRadius: 9, borderWidth: 1, padding: 7 }, timelineComplete: { backgroundColor: '#152629', borderColor: '#315a5e' }, timelineMissed: { backgroundColor: '#111719', borderColor: '#29383c' }, timelineToday: { borderColor: colors.cyan, borderWidth: 1.5 }, timelineTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, timelineWeekday: { color: colors.muted, fontSize: 6.5, fontWeight: '900' }, timelineDate: { color: colors.mint, fontSize: 8, fontWeight: '800', marginTop: 2 }, timelineTodayText: { color: colors.cyan }, timelineScore: { color: colors.yellow, fontSize: 16, lineHeight: 19, fontWeight: '900', marginTop: 5 }, timelineRank: { color: colors.muted, fontSize: 6.5, fontWeight: '700' }, timelineMissedLabel: { color: colors.muted, fontSize: 8, fontWeight: '800', marginTop: 9 }, missedMark: { width: 5, height: 5, borderRadius: 3, backgroundColor: '#607075' }, rankNote: { color: colors.muted, fontSize: 7, marginTop: 7, marginHorizontal: 12 },
