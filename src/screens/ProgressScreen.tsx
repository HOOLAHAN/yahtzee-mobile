...
interface StreakDay { key: string; day: string; date: string; completed: boolean; today: boolean }
const recentStreakDays = (completedDates: string[], todayKey: string, count = 14): StreakDay[] => {
  const completed = new Set(completedDates);
  const [year, month, day] = todayKey.split('-').map(Number);
  const today = new Date(year, month - 1, day, 12);
  return Array.from({ length: count }, (_, index) => {
    const value = new Date(today);
    value.setDate(today.getDate() - (count - 1 - index));
    const key = `${value.getFullYear()}-${String(value.getMonth() + 1).padStart(2, '0')}-${String(value.getDate()).padStart(2, '0')}`;
    return { key, day: value.toLocaleDateString(undefined, { weekday: 'narrow' }), date: String(value.getDate()), completed: completed.has(key), today: key === todayKey };
  });
};
...
export function ProgressScreen({ onCreateAccount }: { onCreateAccount?: () => void }) {
  ...
  const streakDays = useMemo(() => recentStreakDays(dailyDates, localDateKey()), [dailyDates]);
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={loading} onRefresh={() => void load()} tintColor={colors.cyan} />}>
    ...
    <View style={styles.streakHistory}><View style={styles.streakHistoryHeader}><View><Text style={styles.streakHistoryTitle}>Last 14 days</Text><Text style={styles.streakHistoryCopy}>Your Daily Challenge timeline</Text></View><View style={styles.streakLegend}><View style={[styles.legendDot, styles.legendComplete]} /><Text style={styles.legendText}>Played</Text><View style={[styles.legendDot, styles.legendMissed]} /><Text style={styles.legendText}>Missed</Text></View></View><View style={styles.timeline}>{streakDays.map((item) => <View key={item.key} style={styles.timelineDay}><Text style={[styles.timelineWeekday, item.today && styles.timelineTodayText]}>{item.day}</Text><View style={[styles.timelineMarker, item.completed ? styles.timelineComplete : styles.timelineMissed, item.today && styles.timelineToday]}>{item.completed ? <Ionicons name="checkmark" size={13} color={colors.background} /> : <View style={styles.missedMark} />}</View><Text style={[styles.timelineDate, item.today && styles.timelineTodayText]}>{item.date}</Text></View>)}</View></View>
...
const styles = StyleSheet.create({
  ...
  streakHistory: { marginTop: 10, backgroundColor: colors.surface, borderColor: '#293b3f', borderWidth: 1, borderRadius: 14, padding: 12 }, streakHistoryHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 11 }, streakHistoryTitle: { color: colors.white, fontSize: 13, fontWeight: '900' }, streakHistoryCopy: { color: colors.muted, fontSize: 8.5, marginTop: 1 }, streakLegend: { flexDirection: 'row', alignItems: 'center', gap: 4 }, legendDot: { width: 7, height: 7, borderRadius: 4, marginLeft: 4 }, legendComplete: { backgroundColor: colors.yellow }, legendMissed: { backgroundColor: '#334247' }, legendText: { color: colors.muted, fontSize: 7.5 }, timeline: { flexDirection: 'row', justifyContent: 'space-between' }, timelineDay: { flex: 1, alignItems: 'center', gap: 4 }, timelineWeekday: { color: colors.muted, fontSize: 7.5, fontWeight: '800' }, timelineMarker: { width: 20, height: 20, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, timelineComplete: { backgroundColor: colors.yellow }, timelineMissed: { backgroundColor: '#202c2f', borderColor: '#34464a', borderWidth: 1 }, timelineToday: { borderColor: colors.cyan, borderWidth: 2 }, missedMark: { width: 4, height: 4, borderRadius: 2, backgroundColor: '#607075' }, timelineDate: { color: colors.muted, fontSize: 7.5 }, timelineTodayText: { color: colors.cyan, fontWeight: '900' },
